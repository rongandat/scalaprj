...
// TABLES: products, products_to_categories, categories
  function tep_count_products_in_category($category_id, $include_inactive = false) {
	...
	*/
    if ($include_inactive == true) {
      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$category_id . "' and (p.parent_products_id is null or p.child_products_status='1')");
    } else {
      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p.products_status = '1' and p2c.categories_id = '" . (int)$category_id . "' and (p.parent_products_id is null or p.child_products_status='1')");
    }
	//EOF:mod 20120209
...
// TABLES: customers, address_book
  ...
  function tep_get_categories($categories_array = '', $parent_id = '0', $indent = '') {
    ...
    $categories_query = tep_db_query("select c.categories_id, cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where parent_id = '" . (int)$parent_id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and c.categories_id <> 23 order by sort_order, cd.categories_name");
...
function tep_mail2($to_name, $to_email_address, $email_subject, $email_text, $from_email_name = 'Scala Luxury', $from_email_address, $htm=false) 
...
$transport = Swift_SmtpTransport::newInstance('smtp.gmail.com', 465, "ssl")
            ->setUsername('scalaluxury@gmail.com')
            ->setPassword('changed1010!@#');
...
$message = Swift_Message::newInstance($email_subject)
  ...
  ->setFrom(array('info@scalaluxury.com' => $from_email_name))
...
$numSent = $mailer->send($message);
 ...
 function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address, $htm=false, $attachment='',$file_name='',$header='') {
   if (SEND_EMAILS != 'true') return false;
...
function tep_mail_new ($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address, $htm=false, $attachment='',$file_name='',$header='')
{
if (SEND_EMAILS != 'true') return false;
...
//  ->setPassword('changed1010!@#');
$transport = Swift_SmtpTransport::newInstance('smtp.gmail.com', 465, "ssl")
            ->setUsername('info@scalaluxury.com')
            ->setPassword('letinfoin007');
...
$message->setContentType("text/html");
 ...
 if (is_array($attachment)){
     foreach($attachment as $add){
      $file_content=$add;
      $attachment = Swift_Attachment::newInstance($file_content, $file_name);  
...
function get_price_rate($customer_id)
{

if (is_numeric($customer_id))
{

 $query="select customers_group, normal_price from customers where customers_id='$customer_id'";
 $res=tep_db_query($query);
 $row=tep_db_fetch_array($res);

 if ($row['normal_price']=='1') return 100;

 $gid=$row['customers_group'];
 $res=tep_db_query("select group_rate from customers_groups where group_id='$gid'");
 $row=tep_db_fetch_array($res);
 $rate=$row['group_rate'];
}
...
function get_price_group_name($customer_id)
{
...
if (is_numeric($customer_id))
{
 ...
 $gid=$row['customers_group'];
 $res=tep_db_query("select group_name from customers_groups where group_id='$gid'");
 $row=tep_db_fetch_array($res);
...
//huyen - worketc 
function tep_connect_worketc(){
    $alias = WORKETC_ALIAS;
    $email = WORKETC_EMAIL;
    $pass = WORKETC_PASS;
    
    // Start the library. (alias.worketc.com)
    $worketc = new WorkETC(array('alias'=>$alias,'header_method'=>WorkETC::HEADER_INI));
...
function tep_check_lead($email){
    $worketc=tep_connect_worketc();
    $userID = $worketc->session("UserID");
    $allleads=$worketc->GetAllLeadsByOwner(array('EntityID'=>$userID))->Lead;
    foreach($allleads as $alllead){
        $emails[]=$alllead->Relation->Email;
    }
...
function tep_get_entity_by_email($email){
    $worketc=tep_connect_worketc();
    $user=$worketc->GetEntityByEmail(array('email'=>$email));
    if(isset($user)) return $user->EntityID;
    else return 0;
}
...
if (!function_exists('tep_mb_ucfirst')){
	function tep_mb_ucfirst($str, $e='utf-8') {
		return mb_strtoupper(mb_substr($str, 0, 1, $e), $e) . mb_substr($str, 1, mb_strlen($str, $e), $e);
	}
}
if (!function_exists('tep_mb_ucwords')){
	function tep_mb_ucwords($str, $e='utf-8'){
		return mb_convert_case($str, MB_CASE_TITLE, $e);
	}
}
//end huyen - worketc 
//new catalog
function getCusPhone($customer_id){
    $phone='';
    if (is_numeric($customer_id))
    {
        $query="select customers_telephone from customers where customers_id='$customer_id'";
        $res=tep_db_query($query);
        $row=tep_db_fetch_array($res);
...
function getOptionNameById($id){
    global $languages_id;
	$query=tep_db_query('select products_options_values_name from products_options_values where products_options_values_id="'.(int)$id.'" and language_id="'.(int)$languages_id.'"');
	$products_options_name = tep_db_fetch_array($query);
	return $products_options_name['products_options_values_name'];
}
