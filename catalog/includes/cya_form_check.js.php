...
function check_form_account_email(form_name,retype_error) {
    error = false;
    form = form_name;
    error_message = "<?php echo JS_ERROR; ?>";
    
    var email_new = $('#account_email_new').val();
    if (email_new < <?php echo ENTRY_EMAIL_ADDRESS_MIN_LENGTH; ?>){
        error_message = error_message + "* " + <?php echo '"' . ENTRY_EMAIL_ADDRESS_ERROR . '"'; ?> + "\n";
        error = true;
    }
    var email_confirmation = $('#account_email_new_retype').val();

    if (email_new != email_confirmation) {
      error_message = error_message + "* " + retype_error + "\n";
      error = true;
    }
    if (error == true) {
          alert(error_message);
