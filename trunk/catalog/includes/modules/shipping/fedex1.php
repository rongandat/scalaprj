...
/////////////////////////////////////////////////////////
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Max. weight', 'MODULE_SHIPPING_FEDEX1_MAX_WEIGHT', 'NONE', 'Enter the maximum weight when module becomes disabled.', '6', '22', now())");
...
/////////////////////////////////////////////////////////
    ...
    function _CartCount($products_id, $ship_sep='0'){
      ...
      // products price
      if ($_SESSION['ordertype']=="cs")
      $qty = $cart_cs->contents[$products_id]['qty'];
      else
...
//      $product_query = tep_db_query("select products_id, products_price, products_tax_class_id, products_weight from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
     ...
     if ($_SESSION['ordertype']!="cs") {
      ...
      if (isset($cart_cs->contents[$products_id]['attributes'])) {
        reset($cart_cs->contents[$products_id]['attributes']);
        while (list($option, $value) = each($cart_cs->contents[$products_id]['attributes'])) {
          $attribute_price_query = tep_db_query("select options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . (int)$prid . "' and options_id = '" . (int)$option . "' and options_values_id = '" . (int)$value . "'");
          $attribute_price = tep_db_fetch_array($attribute_price_query);
          if ($attribute_price['price_prefix'] == '+') {
            $this->cart_total += $qty * tep_add_tax($attribute_price['options_values_price'], $products_tax);
