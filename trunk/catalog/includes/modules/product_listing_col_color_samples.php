...
*/
    ...
    $thumb_product_with = 115;
    $thumb_product_height = 115;
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_NAME':
            ...
            if (isset($_GET['manufacturers_id'])) {
                ...
                $lc_text = '&nbsp;<a class="cya-name-product" href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $listing[$x]['products_id']) . '">' . $listing[$x]['products_name'] . '</a><p>' .$listing[$x]['products_model'] . '</p><p><a href="#show_popup_add_order_' . $listing[$x]['products_id'] . '" data-id="' . $listing[$x]['products_id'] . '" class="nyroModal_color cya-color-samples-add-order-btn">ADD TO ORDER</a></p>';
            
                //div attributes
                $products_attributes_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_ATTRIBUTES . " patrib where patrib.products_id='" . (int)$listing[$x]['products_id'] . "' and patrib.options_id = popt.products_options_id and popt.language_id = '" . (int)$languages_id . "'");
                $products_attributes = tep_db_fetch_array($products_attributes_query);
                $lc_text .= '<div style="display:none;padding-top:20px;" id="show_popup_add_order_' . $listing[$x]['products_id'] . '">';
                $products_options_name_query = tep_db_query("select distinct popt.products_options_id, popt.products_options_name from " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_ATTRIBUTES . " patrib where patrib.products_id='" . (int)$listing[$x]['products_id'] . "' and patrib.options_id = popt.products_options_id and popt.language_id = '" . (int)$languages_id . "' order by patrib.options_id, patrib.options_values_id");
                //$st=0;
                //$att_str='';
                while ($products_options_name = tep_db_fetch_array($products_options_name_query)) {

                  $products_options_array = array();
                  $products_options_query = tep_db_query("select pov.products_options_values_id, pov.products_options_values_name, pa.options_values_price, pa.price_prefix,pa.stock_number, pa.options_values_width, pa.options_values_height, pa.options_values_depth from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov where pa.products_id = '" . (int)$listing[$x]['products_id'] . "' and pa.options_id = '" . (int)$products_options_name['products_options_id'] . "' and pa.options_values_id = pov.products_options_values_id and pov.language_id = '" . (int)$languages_id . "'");

                  while ($products_options = tep_db_fetch_array($products_options_query)) {

                  if ($products_options['options_values_width']!="" && $products_options['options_values_width']!="0" && $products_options['options_values_width']!="0.00") {
                   if ($width=='' || $width=='0.00') $width=$products_options['options_values_width'];
                   echo '<input name="options_values_width['.$products_options['products_options_values_id'].']" type="hidden" value="'.$products_options['options_values_width'].'">';
                  }

                  if ($products_options['options_values_height']!="" && $products_options['options_values_height']!="0" && $products_options['options_values_height']!="0.00") {
                   if ($height=='' || $height=='0.00') $height=$products_options['options_values_height'];
                   echo '<input name="options_values_height['.$products_options['products_options_values_id'].']" type="hidden" value="'.$products_options['options_values_height'].'">';
                  }

                  if ($products_options['options_values_depth']!="" && $products_options['options_values_depth']!="0" && $products_options['options_values_depth']!="0.00") {
                   if ($depth=='' || $depth=='0.00') $depth=$products_options['options_values_depth'];
                   echo '<input name="options_values_depth['.$products_options['products_options_values_id'].']" type="hidden" value="'.$products_options['options_values_depth'].'">';
                  }
                  ...
                  if ((int)$products_options_name['products_options_id'] == 3) {
                      $add_class_temp = 'product_id3_' . $listing[$x]['products_id'];
                  }
                  $lc_text .= '<span style="margin:15px 0 0 0;display: inline-block;">' . $products_options_name['products_options_name'] . ':' . '</span>';
                  $lc_text .= tep_draw_pull_down_menu('id[' . $products_options_name['products_options_id'] . ']', $products_options_array, $selected_attribute,'class="attr_class ' . $add_class_temp . '"');
                }
                $lc_text .= '<div style="margin:15px 0 0 0;"><a href="javascript:void(0);" class="cya-color-samples-add-order-btn-submit" data-id="' . $listing[$x]['products_id'] . '">Add to Order</a></div>';
                $lc_text .= '</div>';
            }
            break;
