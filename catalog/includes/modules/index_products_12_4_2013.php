...
<?php
  ...
  $lc_text .= '<input type="hidden" value="'.$currencies->display_price($new_products['products_price'], tep_get_tax_rate($new_products['products_tax_class_id']), 1, "", $new_products['products_id']).'" name="product_price['.$new_products['products_id'].']">';
    ...
    $info_box_contents[$row][$col] = array('align' => 'center',
                                           ...
                                           'text' => '&nbsp;<a onclick="fill_quickview(\''.$new_products['products_id'].'\')" href="#quickview" class="nyroModal"><div onmouseover="$(this).show();" onmouseout="$(this).hide();" class="quickview"></div></a><div onmouseover="$(this).prev().find(\'div\').show();" onmouseout="$(this).prev().find(\'div\').hide();"><a class="remote preload" href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $new_products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $new_products['products_image'], $new_products['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div>&nbsp;<br><a class="remote" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $new_products['products_id']) . '">' . $new_products['products_name'] . '</a><br>' . $currencies->display_price($new_products['products_price'], tep_get_tax_rate($new_products['products_tax_class_id']), 1, "", $new_products['products_id'])." ".$lc_text);
