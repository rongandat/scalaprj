...
class mtheme_SidebarGallery_Widget extends WP_Widget {
	function mtheme_SidebarGallery_Widget() {
		...
		$this->WP_Widget('mtheme_sidebar_gallery', MTHEME_WIDGET_PREFIX . ' - Widget Gallery', $widget_ops);
	}
				...
				case "portfolio" :
					...
					?>
						...
						if ($clickaction=="lightbox" && !post_password_required() ) { ?>
							<a rel="prettyPhoto[<?php echo $args['widget_id']; ?>]" href="<?php echo $lightbox_url; ?>" title="<?php echo $imageTitle; ?>">
							<?php
							} else {
							?>
...
add_action('widgets_init', create_function('', 'return register_widget("mtheme_SidebarGallery_Widget");'));
?>