	...
	class tp_widget_recent_tweets extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				MTHEME_WIDGET_PREFIX . ' - Recent Tweets', // Name
				array( 'description' => __( 'Display recent tweets', 'mthemelocal' ), ) // Args
			);
		}
						...
						<div class="mtheme-tweets">
							...
							foreach($tp_twitter_plugin_tweets as $tweet){
								...
								<span class="tweet_avatar">
									<img src="'.$tweet['profile_image_url'].'" width="32" height="32" alt="avatar" />
									<span class="tweet_time">
										<a class="twitter_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.relative_time($tweet['created_at']).'</a>
									</span> 
									<span class="tweet_text">'.convert_links($tweet['text']).'</span>
								</span>
								</li>';
		...
		//widget settings form	
			public function form($instance) {
				...
				<p>
					<code>New Twitter API 1.1 requires all users to signup and generate Consumer and Access Token keys from https://dev.twitter.com/apps</code>
				</p>
...
// register	widget
add_action('widgets_init', create_function('', 'return register_widget("tp_widget_recent_tweets");'));
?>