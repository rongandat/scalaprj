...
*/
    function _draw_stocked_attributes() {
      global $languages_id;
       $product_info_query = tep_db_query("select p.top_coat_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$this->products_id . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
          ...
          for ($a=0; $a<$s; $a++) {
            ...
            elseif ($out_of_stock && ($this->show_out_of_stock != 'True')) {
              unset($attributes[$o]['ovals'][$a]);
                ...
                $stock_item=tep_db_fetch_array($attribute_stock_query);
                $stock_qty=$stock_item['products_stock_quantity'];
                switch ($this->mark_out_of_stock) {
                case 'Left':   $attributes[$o]['ovals'][$a]['text']='Stock '.$stock_qty.' - '.$attributes[$o]['ovals'][$a]['text'];
                               break;
                case 'Right':  $attributes[$o]['ovals'][$a]['text'].=' - Stock '.$stock_qty;
                               break;
              }
            }
          }
		  if($product_info['top_coat_price']==1){
				if($attributes[$o]['oid']==3){
						$attributes[$o]['oname']='Top Coat Option';
						$out.='<tr><td align="right" class=main><b>'.$attributes[$o]['oname'].":</b></td><td class=main>".tep_draw_pull_down_menu('id['.$attributes[$o]['oid'].']',array_values($attributes[$o]['ovals']),$attributes[$o]['default'], "onchange=\"stkmsg(this.form);\" class=\"attr_class uni\"")."</td></tr>\n";
					}
		  }
		  else 
			$out.='<tr><td align="right" class=main><b>'.$attributes[$o]['oname'].":</b></td><td class=main>".tep_draw_pull_down_menu('id['.$attributes[$o]['oid'].']',array_values($attributes[$o]['ovals']),$attributes[$o]['default'], "onchange=\"stkmsg(this.form);\" class=\"attr_class uni\"")."</td></tr>\n";
