...
class mtheme_Social_Widget extends WP_Widget {
	function mtheme_Social_Widget() {
		...
		$this->WP_Widget( 'msocial-widget', MTHEME_WIDGET_PREFIX . __(' - Social Widget', 'mthemelocal'), $widget_ops,$control_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		for ($social_count=1; $social_count <=12; $social_count++ ) {
		...
		for ($social_count=1; $social_count <=12; $social_count++ ) {
			...
			if ( $customurl[$social_count] != '' && $customname[$social_count] != '' && $customicon[$social_count] != '' ) {
				...
				<a class="ntips" original-title="<?php echo $customname[$social_count]; ?>" href="<?php echo $customurl[$social_count]; ?>" <?php echo $nofollow; ?> <?php echo $newtab; ?>>
					<i class="icon-<?php echo $customicon[$social_count]; ?>"></i>
				</a>
				</li>
				<?php
			} else {
...
<!-- Open in new tab: Dropdown -->
		...
		<p>
			...
			<input class="widefat" type="text" id="<?php echo $this->get_field_id( 'call_us' ); ?>" name="<?php echo $this->get_field_name( 'call_us' ); ?>" value="<?php echo $instance['call_us']; ?>" />
		</p>
		<!-- Call us link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'call_us_link' ); ?>"><?php _e('Contact text link:', 'mthemelocal'); ?></label><br />
			<input class="widefat" type="text" id="<?php echo $this->get_field_id( 'call_us_link' ); ?>" name="<?php echo $this->get_field_name( 'call_us_link' ); ?>" value="<?php echo $instance['call_us_link']; ?>" />
		</p>		
		...
		<p>
			...
			<input class="widefat" type="text" id="<?php echo $this->get_field_id( 'custom'.$social_count.'url' ); ?>" name="<?php echo $this->get_field_name( 'custom'.$social_count.'url' ); ?>" value="<?php echo $instance['custom'.$social_count.'url']; ?>" />
		</p>
		<?php
		}
		?>
