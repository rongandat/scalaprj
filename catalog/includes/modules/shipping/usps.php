...
class usps
{
	...
	if ($order->delivery['country']['id'] == SHIPPING_ORIGIN_COUNTRY)
		...
		{
		$request  = '<RateV4Request USERID="' . $Authentication[0] . '" PASSWORD="312EO17BW915">' .
					'<Revision>2</Revision>';
		$services_count = 0;
		...
		if ($http->Get('/' . $api_dll . '?' . $request)) $body = $http->getBody();
				#mail('fuzzoid@gmail.com','USPS Rate Quote response',$body,'From: <user@localhost.com>');  
		...
		if ($http->Get('/' . $api_dll . '?' . $value)) $transresp[$key] = $http->getBody();
				#mail('fuzzoid@gmail.com','USPS Transit Response',$transresp[$key],'From: <user@localhost.com>');
		}
		}
		$http->Disconnect();
		} else {
