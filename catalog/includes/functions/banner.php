...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    ...
    if (tep_not_null($banner['banners_html_text'])) {
      ...
      $banner_string = '<a class="remote" href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">' . mm_output_flash_movie( $banner['banners_title'], DIR_WS_IMAGES . $banner['banners_image'] , $size[0]  , $size[1]) . '</a>';
	  } else {
	  $banner_string = '<a style="padding:0px;" class="remote" href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">' . tep_image(DIR_WS_IMAGES . $banner['banners_image'], $banner['banners_title']) . '</a>';
	  }
	}
