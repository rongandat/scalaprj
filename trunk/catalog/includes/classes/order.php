...
*/
  ...
  class order {
    ...
    function query($order_id) {
      ...
      $order_query = tep_db_query("select customers_id, customers_name, tax_country_id, tax_zone_id, customers_company, customers_street_address, customers_suburb, customers_city, customers_postcode, customers_state, customers_country, customers_telephone, customers_email_address, customers_address_format_id, delivery_name, delivery_company, delivery_street_address, delivery_suburb, delivery_city, delivery_postcode, delivery_state, delivery_country, delivery_address_format_id, billing_name, billing_company, billing_street_address, billing_suburb, billing_city, billing_postcode, billing_state, billing_country, billing_address_format_id, payment_method, cc_type, cc_owner, cc_number, cc_expires, currency, currency_value, date_purchased, orders_status, last_modified, ship_via, rep, side_mark, delivery_time from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
      ...
      $this->info = array('currency' => $order['currency'],
		          ...
		          'tax_country_id' => $order['tax_country_id'],
		          'tax_zone_id' => $order['tax_zone_id'],
                          'ship_via' => $order['ship_via'],
                          'side_mark' => $order['side_mark'],
    ...
    function cart() {
      global $HTTP_POST_VARS, $customer_id, $sendto, $billto, $cart, $cart_cs, $cart_pr, $languages_id, $currency, $currencies, $shipping, $payment, $comments, $customer_default_address_id;

      if ($_SESSION['ordertype']=='cs') {
       $this->content_type = $cart_cs->get_content_type();
      }
      elseif ($_SESSION['ordertype']=='pr') {

       $this->content_type = $cart_pr->get_content_type();
      }
      else {
      ...
      elseif ($_SESSION['ordertype']=='pr') 
      $products = $cart_pr->get_products();
      else
      $products = $cart->get_products();

      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      if ($_SESSION['ordertype']=='cs') 
       $att=$cart_cs->attributes_price($products[$i]['id']);
      elseif ($_SESSION['ordertype']=='pr') 
       $att=$cart_pr->attributes_price($products[$i]['id']);
      else
       $att=$cart->attributes_price($products[$i]['id']);

        $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                        'tax_class' => $products[$i]['tax_class_id'],  
                                        'name' => $products[$i]['name'],
                                        'model' => $products[$i]['model'],
                                        'tax' => tep_get_tax_rate($products[$i]['tax_class_id'], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
                                        'tax_description' => tep_get_tax_description($products[$i]['tax_class_id'], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
                                        'price' => $products[$i]['price'],
                                        'final_price' => $products[$i]['price'] + $att,
        ...
        if ($products[$i]['attributes']) {
          ...
          while (list($option, $value) = each($products[$i]['attributes'])) {
            $attributes_query = tep_db_query("select popt.products_options_track_stock, popt.products_options_id, poval.products_options_values_id, popt.products_options_name, poval.products_options_values_name, pa.options_values_price, pa.price_prefix from " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_OPTIONS_VALUES . " poval, " . TABLE_PRODUCTS_ATTRIBUTES . " pa where pa.products_id = '" . (int)$products[$i]['id'] . "' and pa.options_id = '" . (int)$option . "' and pa.options_id = popt.products_options_id and pa.options_values_id = '" . (int)$value . "' and pa.options_values_id = poval.products_options_values_id and popt.language_id = '" . (int)$languages_id . "' and poval.language_id = '" . (int)$languages_id . "'");
        ...
        $shown_price = $currencies->calculate_price($this->products[$index]['final_price'], $this->products[$index]['tax'], $this->products[$index]['qty'], "", (int)$this->products[$index]['id']);
